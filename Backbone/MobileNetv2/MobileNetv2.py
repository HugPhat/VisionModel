from .utils import download_origin, load_pretrained_weight
import torch
...
class MobileNetv2(nn.Module):
    def __init__(self, input_size=224, n_classes=10, debug=False, model_name='mbv2'):
    ...
    def constructNet(self):
        ...
        for t, c, n, s in self.model_hyperparams:
            for block in range(n):
                ...
                if self.debug:
                    print(
                        f'[{n}] with Id {Id}==> block {block} : [in]: {prev_filter}, [out]: {c}')
        ...
        if self.debug:
            print(
                f'[1] with Id {Id}==> block {block} : [in]: {prev_filter}, [out]: {self.last_channel}')
            print(
                f'[1] with Id {Id + 1}==> block {block} : [in]: {self.last_channel}, [out]: {self.n_classes}')
    ...
    def predict(self, img, cuda=False):
        ...
        Args:
            img ([Pillow Image]): [RGB image]
            cuda (bool, optional): [inference with GPU or CPU]. Defaults to False.
        Returns:
            [list]: [class index, confidence score]
        """
        self.eval()
        ...
        clss = torch.argmax(out).cpu().item()
        conf = torch.max(out).cpu().item()
        return [clss, round(conf, 3)]


def mobilenetv2(n_classes=10, debug=False, create_new=True, name='mbv2'):
    model = MobileNetv2(n_classes=1000, model_name=name)
    origin_weight = download_origin(model_urls['mobilenet_v2'])
    load_pretrained_weight(model, origin_weight, False)
    if create_new:
        model.classifier = nn.Linear(model.last_channel, n_classes)
